...
1. Run the `ui-ux-pro-max` design-system generator to get style + color tokens + typography +
   UX anti-patterns:
   ```bash
   python .claude/skills/ui-ux-pro-max/scripts/search.py "$ARGUMENTS" --design-system -p "RepoVeriX"
   ```
   (Resolve the skill path from `.claude/skills/ui-ux-pro-max/scripts/search.py` — the skill
    is already installed locally; no CLI install needed.)
