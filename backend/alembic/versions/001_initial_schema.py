...
def upgrade() -> None:
    ...
    scan_configuration = postgresql.ENUM(
        "static_only", "llm_only", "static_llm", "repoverix", name="scan_configuration", create_type=False
    )
    analysis_stage = postgresql.ENUM(
        "ingestion",
        "parsing",
        "static_analysis",
        "knowledge_graph",
    ...
    symbol_kind = postgresql.ENUM(
        "function", "class", "method", "variable", "import", name="symbol_kind", create_type=False
    )
    ...
    finding_status = postgresql.ENUM(
        "verified", "probable", "rejected", name="finding_status", create_type=False
    )
    ...
    patch_status = postgresql.ENUM(
        "candidate", "applied", "verified", "failed", "not_verified", name="patch_status", create_type=False
    )
    ...
    test_outcome = postgresql.ENUM(
        "passed", "failed", "error", "skipped", name="test_outcome", create_type=False
    )
    ...
    op.create_table(
        "users",
        sa.Column(
            "id", postgresql.UUID(as_uuid=True), primary_key=True, default=sa.text("gen_random_uuid()")
        ),
